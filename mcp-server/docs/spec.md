...
```jsonc
{
  ...
  "diagnostics": {
    ...
    "capture_mode_reason": "Page height 4200px exceeds 2x viewport. Full page available via capture_mode='full'."
  }
}
```
...
```jsonc
{
  ...
  "steps": [
    ...
    // fill: 入力フィールドに値を入力
    { "action": "fill", "ref": 5, "value": "test@example.com" },

    // click with description: 説明付きクリック（注釈ラベルに使用）
    { "action": "click", "ref": 7, "description": "ログインボタン押下" },
